...
export const linkMessageToWorkspace = async ({
  workspaceId,
  messageId,
}: {
  workspaceId: string;
  ...
  const link = await db
    .insert(workspacesToMessages)
    .values({ workspaceId, messageId })
    .onConflictDoNothing();

  await db
    .update(messages)
    .set({
      workspaceIds: sql`array_append(${messages.workspaceIds}, ${workspaceId})`,
    })
    .where(eq(messages.id, messageId));
