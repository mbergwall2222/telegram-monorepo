...
import type {
  ...
  MessageIDLike,
  MessageLike,
} from "@telegram/gramjs/dist/define";
import { RequestIter } from "@telegram/gramjs/dist/requestIter";
import {
  _EntityType,
  _entityType,
  TotalList,
...
} from "@telegram/gramjs/dist/Helpers";
import {
  getInputMedia,
  getMessageId,
  getPeerId,
  parseID,
} from "@telegram/gramjs/dist/Utils";
import type { TelegramClient } from "@telegram/gramjs";
import { utils } from "@telegram/gramjs";
import { messageParse, users, uploads } from "@telegram/gramjs/dist/client";
import { bigInt } from "@telegram/gramjs";
...
export class _IDsIter extends RequestIter {
  ...
  async _loadNextChunk() {
    ...
    if (this._ty == _EntityType.CHANNEL) {
      ...
      if (this._entity) {
        fromId = await users._getPeer(this.client, this._entity);
      }
    }
    if (r instanceof Api.messages.MessagesNotModified) {
...
export async function sendMessage(
  ...
  if (message && message instanceof Api.Message) {
    ...
    if (formattingEntities == undefined) {
      [message, formattingEntities] = await messageParse._parseMessageText(
        client,
        message || "",
        parseMode
      );
  ...
  if (result instanceof Api.UpdateShortSentMessage) {
    const msg = new Api.Message({
      id: result.id,
      peerId: await users._getPeer(client, entity),
      message: message,
      date: result.date,
      out: result.out,
      media: result.media,
...
export async function editMessage(
  ...
  if (file) {
    const { fileHandle, media, image } = await uploads._fileToMedia(client, {
      file,
      forceDocument,
    });
    inputMedia = media;
  ...
  if (message instanceof Api.Message) {
    ...
    id = message;
    if (formattingEntities == undefined) {
      [text, entities] = await messageParse._parseMessageText(
        client,
        text || "",
        parseMode
      );
